...
// #############################################################################
// #  AXIOMS FOR NON-DETERMINISTIC ITERATOR
// #############################################################################
...
// Model functions for each 'get' call, providing the "havoc" sequences.
function model_get_AID(iter: Iterator Activity, index: int) returns (UUID);
function model_get_UID(iter: Iterator Activity, index: int) returns (int);

// --- Public Intrinsic Functions ---
function scan<T>(t: Table T) returns (Iterator T);
...
// --- Iterator Axioms ---
// scan()
axiom (forall<T> t: Table T :: iter_position(scan(t)) == 0);
axiom (forall<T> t: Table T :: iter_length(scan(t)) >= 0);
// This axiom makes sure that we only run loop once.
// axiom (forall<T> t: Table T :: iter_length(scan(t)) == 1);
...
axiom (forall iter: Iterator Activity, index: int ::
    model_get_AID(next(iter), index) == model_get_AID(iter, index) &&
    model_get_UID(next(iter), index) == model_get_UID(iter, index)
);
...
// get_...()
axiom (forall iter: Iterator Activity :: hasNext(iter) ==> get_AID(iter) == model_get_AID(iter, iter_position(iter)));
axiom (forall iter: Iterator Activity :: hasNext(iter) ==> get_UID(iter) == model_get_UID(iter, iter_position(iter)));
  ...
  modifies Activity_Log;
{
  ...
  var s1_uid: int;
  var s1_activity_iter_base: Iterator Activity;
  ...
  // Create the base iterator ONCE. Both runs will use this same instance.
  s1_activity_iter_base := scan(TBL_Activity);
  ...
  s1_block14_run1:
    ...
    s1_activity_iter_run1 := s1_activity_iter_base;
...
second_execution_start:
  ...
  s1_block14_run2:
    ...
    s1_activity_iter_run2 := s1_activity_iter_base;
