...
use super::{ReplCommand, RuntimeError, RuntimeState};
use crate::cfg::CfgProgram;
use clap_repl::reedline::{
    Completer, Prompt, PromptEditMode, PromptHistorySearch, Span, Suggestion,
};
...
pub fn start_runtime_repl() -> Result<(), String> {
    ...
    println!(
        "Type {} for commands, {} for completion, {} or {} to quit",
        "'help'".bright_cyan(),
        "TAB".bright_yellow(),
    ...
    loop {
        match editor.read_command() {
            ReadCommandOutput::Command(command) => match &command {
                ReplCommand::Exit => {
                    println!("Goodbye!");
                    break;
                }
...
pub fn start_runtime_repl_with_cfg(cfg_program: CfgProgram) -> Result<(), String> {
    ...
    println!(
        "Loaded optimized CFG with {} functions and {} tables",
    ...
    runtime_state
        .load_cfg(cfg_program)
        .map_err(|e| format!("Failed to load CFG: {}", e))?;
