...
use crate::{
    ...
    cfg::{CfgBuilder, CfgProgram, FieldId, FunctionId, TableId},
};
use colored::*;
...
impl RuntimeState {
    ...
    pub fn load_file(&mut self, file_path: &str) -> Result<(), RuntimeError> {
        ...
        println!(
            "{} Loaded program with {} functions, {} tables",
            "SUCCESS:".green().bold(),
    ...
    pub fn get_table_data(
        &self,
        table_name: &str,
    ) -> Option<&HashMap<Vec<RuntimeValue>, HashMap<FieldId, RuntimeValue>>> {
    ...
    pub fn clear_data(&mut self) -> Result<(), RuntimeError> {
        ...
        println!(
            "{} Cleared all table data - database reset to empty state",
            "SUCCESS:".green().bold()
        );
        Ok(())
    }
