...
use crate::cfg::{FunctionId, Statement, Operand, Rvalue, BinaryOp, UnaryOp, Constant};
use super::{RuntimeState, RuntimeValue, RuntimeError};
...
pub fn execute_function(
    ...
    let (statements_to_execute, parameters) = {
        let cfg = state.cfg_program.as_ref()
...
fn execute_statement_isolated(
    ...
    match stmt {
        Statement::Assign { var, rvalue, span: _ } => {
            // Get variable name
            let var_name = if let Some(cfg) = &state.cfg_program {
                cfg.functions[func_id].variables[*var].name.clone()
            } else {
                return Err(RuntimeError::ExecutionError("No program loaded".to_string()));
            };
        ...
        Statement::TableAssign { table, pk_fields: _, pk_values, field, value, span: _ } => {
            ...
            // Update table
            let table_data = state.table_data.get_mut(table)
                .ok_or_else(|| RuntimeError::ExecutionError(format!("Table {:?} not found", table)))?;
            
            let record = table_data.entry(pk_vals.clone()).or_insert_with(HashMap::new);
...
fn evaluate_rvalue_isolated(
    ...
    match rvalue {
        ...
        Rvalue::TableAccess { pk_values, field, .. } => {
            // Get primary key values  
            let mut pk_vals = Vec::new();
            for operand in pk_values {
                let val = evaluate_operand_isolated(operand, local_vars, state, func_id)?;
            ...
            if let Some((_, table_data)) = state.table_data.iter().next() {
                let value = table_data.get(&pk_vals)
        ...
        Rvalue::BinaryOp { op, left, right } => {
            ...
            match (op, &left_val, &right_val) {
                (BinaryOp::Add, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Int(a + b)),
                (BinaryOp::Sub, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Int(a - b)),
                (BinaryOp::Mul, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Int(a * b)),
                ...
                (BinaryOp::Lt, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Bool(a < b)),
                (BinaryOp::Lte, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Bool(a <= b)),
                (BinaryOp::Gt, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Bool(a > b)),
                (BinaryOp::Gte, RuntimeValue::Int(a), RuntimeValue::Int(b)) => Ok(RuntimeValue::Bool(a >= b)),
                _ => Err(RuntimeError::ExecutionError(format!("Unsupported binary operation: {:?} {:?} {:?}", left_val, op, right_val))),
            }
        }
        ...
        Rvalue::UnaryOp { op, operand } => {
            ...
            match (op, &val) {
                ...
                _ => Err(RuntimeError::ExecutionError(format!("Unsupported unary operation: {:?} {:?}", op, val))),
            }
        }
    }
}
    ...
    match operand {
        ...
        Operand::Var(var_id) => {
            ...
            local_vars.get(&var_name)
                .cloned()
                .ok_or_else(|| RuntimeError::ExecutionError(format!("Variable '{}' not found", var_name)))
        }
    }
}