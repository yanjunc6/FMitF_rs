...
use crate::cfg::{BasicBlock, Function as FunctionCfg, HopId, Instruction};
...
pub trait VerificationStrategy {
    ...
    fn generate_block_edges(
        ...
        self.base().default_generate_block_edges(block, slice_id)
    }
    ...
    fn walk_and_verify(&mut self, function: &FunctionCfg) -> Results<BoogieProcedure> {
        ...
        if let Some(entry_hop) = function.entry_hop {
            ...
            loop {
                let (block_ids, next_hop_opt) = {
                    let base = self.base();
                    let hop = &base.cfg_program.hops[current_hop_id];
                    let last_block_id = *hop.blocks.last().unwrap();
                    let terminator = &base.cfg_program.basic_blocks[last_block_id].terminator;
                ...
                for &block_id in &block_ids {
                    let block = self.base().cfg_program.basic_blocks[block_id].clone();
                    let label;
                    {
                    ...
                    for instruction in &block.instructions {
                        ...
                        let boogie_lines = self
                            .base()
                            .convert_instruction(instruction, current_hop_id.index())?;
                    ...
                    let edge_lines = self.generate_block_edges(&block, current_hop_id.index())?;
