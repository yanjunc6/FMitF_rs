    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CrossNodeAccess { table, table_node, current_node } =>
                write!(f, "Cannot access table '{}' on node '{}' from node '{}'", 
                       table, table_node, current_node),
            Self::InvalidPrimaryKey { table, column } =>
                write!(f, "Column '{}' is not the primary key of table '{}'", column, table),
            Self::AbortNotInFirstHop { function, hop_index } =>
                write!(f, "Abort statement not allowed in hop {} of function '{}' (only in first hop)", 
                       hop_index, function),
        }
    }
}
