...
impl<'a> Printer<'a> {
    ...
    fn print_functions(&self, functions: &[Rc<FunctionDeclaration>]) {
        ...
        for (i, func) in functions.iter().enumerate() {
            ...
            self.with_depth(self.depth + 2)
                .print_parameters(&func.parameters);
    ...
    fn print_hops(&self, hops: &[Rc<HopBlock>]) {
        ...
        for (i, hop) in hops.iter().enumerate() {
            ...
            self.with_depth(self.depth + 2)
                .print_statements(&hop.statements);
        }
    }
    ...
    fn print_statements(&self, statements: &[Statement]) {
        ...
        for (i, stmt) in statements.iter().enumerate() {
            self.with_depth(self.depth + 1)
                .print_statement_with_index(i, stmt);
        }
    }
    ...
    fn print_statement_with_index(&self, index: usize, stmt: &Statement) {
        match &stmt.node {
            ...
            StatementKind::Abort(_) => {
                // NEW: Add abort statement printing
                println!(
                    "{}[{}] AbortStatement{}",
                    self.indent(),
                    index,
            ...
            StatementKind::IfStmt(i) => {
                ...
                self.with_depth(self.depth + 2)
                    .print_statements(&i.then_branch);
                println!("{}else_branch:", self.with_depth(self.depth + 1).indent());
                match &i.else_branch {
                    Some(else_stmts) => {
                        self.with_depth(self.depth + 2).print_statements(else_stmts)
                    }
                    None => println!("{}None", self.with_depth(self.depth + 2).indent()),
                }
            }
    ...
    fn print_expression(&self, expr: &Expression) {
        ...
        self.with_depth(self.depth + 1)
            .print_expression_kind(&expr.node);
    }
