...
use crate::ast::{
    Program, PartitionDeclaration, TableDeclaration, FunctionDeclaration,
    HopBlock, Statement, Expression, StatementKind, ExpressionKind,
    VarDecl, TypeName, ReturnType, HopType, AssignmentOperator, LValue, BinaryOp, UnaryOp,
    PartitionId, TableId, FunctionId, HopId, VarId,
};
use std::io::Write;
...
impl AstPrinter {
    ...
    fn print_partition(&self, partition: &PartitionDeclaration, partition_id: PartitionId, program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        write!(writer, "partition {} (id: {:?})", partition.name, partition_id)?;
        ...
        for (i, &param_id) in partition.parameters.iter().enumerate() {
            ...
            if let Some(param) = program.parameters.get(param_id) {
                write!(writer, "{} {}", self.format_type(&param.param_type), param.param_name)?;
            }
        }
        write!(writer, ")")?;
    ...
    fn print_constant(&self, const_var: &VarDecl, const_id: VarId, _program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        write!(writer, "const {} {} (id: {:?})", self.format_type(&const_var.ty), const_var.name, const_id)?;
    ...
    fn print_table(&self, table: &TableDeclaration, table_id: TableId, program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        ...
        for &field_id in &table.fields {
            if let Some(field) = program.fields.get(field_id) {
                ...
                writeln!(writer, "{} {}{};", self.format_type(&field.field_type), field.field_name, primary_marker)?;
            }
        }
    ...
    fn print_function(&self, function: &FunctionDeclaration, func_id: FunctionId, program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        write!(writer, "{} {} (id: {:?})", 
    ...
    /// Print a hop block
    fn print_hop(&self, hop: &HopBlock, hop_id: HopId, hop_index: usize, program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        ...
        match &hop.hop_type {
            ...
            HopType::ForLoop { loop_var, loop_var_type, start, end } => {
                write!(writer, "hops for {} {} = ", self.format_type(loop_var_type), loop_var)?;
    ...
    /// Print a statement
    fn print_statement(&self, stmt: &Statement, program: &Program, writer: &mut dyn Write, level: usize) -> std::io::Result<()> {
        self.write_indent(writer, level)?;
        ...
        match &stmt.node {
            ...
            StatementKind::VarDecl(var_decl) => {
                write!(writer, "{} {} = ", self.format_type(&var_decl.var_type), var_decl.var_name)?;
            ...
            StatementKind::ForStmt(for_stmt) => {
                write!(writer, "for ({} {} = ", self.format_type(&for_stmt.loop_var_type), for_stmt.loop_var)?;
    ...
    /// Print an lvalue
    fn print_lvalue(&self, lvalue: &LValue, program: &Program, writer: &mut dyn Write) -> std::io::Result<()> {
        match lvalue {
            ...
            LValue::TableField { table_name, pk_exprs, field_name, .. } => {
            ...
            LValue::TableRecord { table_name, pk_exprs, .. } => {
                write!(writer, "{}[", table_name)?;
                for (i, &pk_expr_id) in pk_exprs.iter().enumerate() {
                    if i > 0 { write!(writer, ", ")?; }
                    if let Some(expr) = program.expressions.get(pk_expr_id) {
            ...
            LValue::ArrayElement { array_name, index, .. } => {
    ...
    /// Print an expression
    fn print_expression(&self, expr: &Expression, program: &Program, writer: &mut dyn Write) -> std::io::Result<()> {
        match &expr.node {
            ...
            ExpressionKind::TableFieldAccess { table_name, pk_exprs, field_name, .. } => {
                write!(writer, "{}[", table_name)?;
                for (i, &pk_expr_id) in pk_exprs.iter().enumerate() {
                    if i > 0 { write!(writer, ", ")?; }
                    if let Some(expr) = program.expressions.get(pk_expr_id) {
            ...
            ExpressionKind::TableAccess { table_name, pk_exprs, .. } => {
                write!(writer, "{}[", table_name)?;
                for (i, &pk_expr_id) in pk_exprs.iter().enumerate() {
                    if i > 0 { write!(writer, ", ")?; }
                    if let Some(expr) = program.expressions.get(pk_expr_id) {
            ...
            ExpressionKind::ArrayAccess { array_name, index, .. } => {
                write!(writer, "{}[", array_name)?;
                if let Some(idx_expr) = program.expressions.get(*index) {
                    self.print_expression(idx_expr, program, writer)?;
                }
...
pub fn print_program_to_writer(program: &Program, options: &PrintOptions, writer: &mut dyn Write) -> std::io::Result<()> {
