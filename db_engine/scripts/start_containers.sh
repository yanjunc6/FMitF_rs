...
for idx in `seq 1 ${nodes}`; do
  ...
  docker run -d --publish-all=true --cap-add=SYS_ADMIN --cap-add=NET_ADMIN --security-opt seccomp:unconfined --cpuset-cpus=$CPUIDS --tmpfs /root/ShardDB:exec --name=$PREFIX$idx $IMGNAME tail -f /dev/null 2>&1 >> $DFILE
done
